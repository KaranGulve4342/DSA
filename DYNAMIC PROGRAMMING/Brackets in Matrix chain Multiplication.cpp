...
public:
    string matrixChainOrder(int p[], int n){
        ...
        string result = generateParenthesis(s, 1, n - 1);
        return result;
    }

    string generateParenthesis(vector<vector<int>>& s, int i, int j) {
        if (i == j) {
            return string(1, 'A' + i - 1);
        }

        return "(" + generateParenthesis(s, i, s[i][j]) + generateParenthesis(s, s[i][j] + 1, j) + ")";
    }
};
