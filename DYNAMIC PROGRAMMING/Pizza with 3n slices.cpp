...
int solveTab(vector<int>& slices){
    ...
    for(int index = k-1;index >= 1;index--){
        for(int n = 1;n <= k/3;n++){
            ...
            int skip = 0 + dp2[index + 1][n];

            dp2[index][n] = max(take, skip);
        }
    }
    int case2 = dp2[1][k/3];
...
int solveMem(int index, int endIndex, vector<int>& slices, int n, vector<vector<int> >& dp){
    ...
    int skip = 0 + solvemem(index + 1, endIndex, slices, n, dp);

    return dp[index][n] = max(take, skip);
}
...
public:
    int solve(int index, int endIndex, vector<int>& slices, int n){
        ...
        int skip = 0 + solve(index + 1, endIndex, slices, n);
